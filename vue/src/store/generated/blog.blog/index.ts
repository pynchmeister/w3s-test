...
export default {
	...
	actions: {
		...
		async sendMsgUploadFilecoin({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.BlogBlog.tx.sendMsgUploadFilecoin({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgUploadFilecoin:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgUploadFilecoin:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
		...
		async MsgUploadFilecoin({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.BlogBlog.tx.msgUploadFilecoin({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgUploadFilecoin:Init Could not initialize signing client. Wallet is required.')
				} else{
					throw new Error('TxClient:MsgUploadFilecoin:Create Could not create message: ' + e.message)
				}
			}
		},
